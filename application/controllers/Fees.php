        ...
        if ($_POST) {
            if (get_permission('due_invoice', 'is_view')) {
                ...
                if (empty($submit_btn)) {
                    ...
                    $fees_type_post = (string)$this->input->post('fees_type');
                    $feegroup = explode("|", $fees_type_post);
                    $feegroup_id = isset($feegroup[0]) ? $feegroup[0] : '';
                    $fee_feetype_id = isset($feegroup[1]) ? $feegroup[1] : '';
