...
class Feespayment extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library('cashfree_payment');
    }
    ...
    public function checkout()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($payVia == 'cashfree') {
                    $url = base_url("feespayment/cashfree");
                    $this->session->set_userdata("params", $params);
                }
    ...
    public function cashfree()
    {
        $params = $this->session->userdata('params');
        if (!empty($params)) {
            $params['amount'] = floatval($params['amount'] + $params['fine']);
            $stu = $this->userrole_model->getStudentDetails();
            $params['student_phone'] = $stu['mobileno'];
            $response = $this->cashfree_payment->payment($params);
            if (isset($response['payment_session_id'])) {
                $data['payment_session_id'] = $response['payment_session_id'];
                $data['order_id'] = $response['order_id'];
                $config = $this->get_payment_config();
                $data['environment'] = ($config['cashfree_sandbox'] == 1) ? "sandbox" : "production";
                $this->load->view('layout/cashfree_pay', $data);
            } else {
    ...
    public function cashfree_verify()
    {
        ...
        $params = $this->session->userdata('params');
        if (!empty($params) && !empty($order_id)) {
            $response = $this->cashfree_payment->verify($order_id);
            if (isset($response['order_status']) && $response['order_status'] == 'PAID') {
                ...
                $arrayFees = array(
                    ...
                    'pay_via' => 21,
                    'collect_by' => 'online',
                    'remarks' => "Fees deposits online via Cashfree TxnID: " . $order_id,
                    'date' => date("Y-m-d"),
                );
                $this->savePaymentData($arrayFees);
                set_alert('success', translate('payment_successfull'));
    ...
    public function cashfree_webhook()
    {
        ...
        // This would process the JSON payload sent by Cashfree
        $raw_post_data = file_get_contents('php://input');
        $data = json_decode($raw_post_data, true);
        
        if (isset($data['order']['order_id']) && $data['type'] == 'PAYMENT_SUCCESS_WEBHOOK') {
             // Verification logic here (usually checking signatures)
             // For this project, we'd need to find the student/params associated with this order
             // Typically we store order_id in a separate table to link it back
        }
    }
