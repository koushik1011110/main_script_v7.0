...
class Saas extends MY_Controller
{
    ...
    public function payment_active()
    {
        ...
        $cashfree_status = isset($_POST['cashfree_status']) ? 1 : 0;
        $arrayData = array(
    ...
    {
        if ($_POST) {
            $ignoreArray = [$this->security->get_csrf_token_name(), 'facebook_url', 'twitter_url', 'linkedin_url', 'instagram_url', 'youtube_url', 'google_plus', 'old_payment_logo', 'old_slider_image', 'old_overly_image', 'terms_and_conditions', 'agree_checkbox_text', 'overly_image_status', 'old_slider_bg_image', 'button_text_1', 'button_url_1', 'button_text_2', 'button_url_2'];
    ...
    public function emailconfig()
    {
        if ($this->input->post('submit') == 'update') {
            ...
            foreach ($this->input->post() as $key => $value) {
                if ($key == 'submit' || $key == $this->security->get_csrf_token_name()) {
                    continue;
                }
                $data[$key] = $value;
            }
