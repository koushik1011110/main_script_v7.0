...
class Settings extends Admin_Controller
{
    ...
    public function cashfree_save()
    {
        ...
        $branchID = $this->application_model->get_branch_id();
        $this->form_validation->set_rules('cashfree_app_id', 'App ID', 'trim|required');
        $this->form_validation->set_rules('cashfree_secret_key', 'Secret Key', 'trim|required');
        if ($this->form_validation->run() !== false) {
            $sandbox = isset($_POST['cashfree_sandbox']) ? 1 : 0;
            $arrayConfig = array(
                'cashfree_app_id' => $this->input->post('cashfree_app_id'),
                'cashfree_secret_key' => $this->input->post('cashfree_secret_key'),
                'cashfree_sandbox' => $sandbox,
                'branch_id' => $branchID,
            );
            $this->settings_model->addPayment($arrayConfig);
