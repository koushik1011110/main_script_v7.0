...
class Dashboard_model extends CI_Model
{
    ...
    public function getTotalFeesCollected($branchID = '')
    {
    ...
    public function getTodaysFeesCollected($branchID = '')
    {
        $this->db->select('IFNULL(SUM(h.amount), 0) as total');
        $this->db->from('fee_payment_history as h');
    ...
    public function getPendingFeesDues($branchID = '')
    {
        $sessionID = get_session_id();
        $this->db->select('IFNULL(SUM(gd.amount), 0) as total_allocated');
        $this->db->from('fee_allocation as fa');
        $this->db->join('fee_groups_details as gd', 'gd.fee_groups_id = fa.group_id', 'inner');
        $this->db->where('fa.session_id', $sessionID);
        if (!empty($branchID)) {
            $this->db->where('fa.branch_id', $branchID);
        }
        $allocated = $this->db->get()->row()->total_allocated;

        $this->db->select('IFNULL(SUM(h.amount + h.discount), 0) as total_paid');
        $this->db->from('fee_payment_history as h');
        if (!empty($branchID)) {
            $this->db->join('fee_allocation as fa', 'fa.id = h.allocation_id', 'inner');
            $this->db->where('fa.branch_id', $branchID);
        }
        $paid = $this->db->get()->row()->total_paid;
