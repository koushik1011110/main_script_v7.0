...
$enroll_id = isset($basic['enroll_id']) ? $basic['enroll_id'] : 0;
$allocations = $this->fees_model->getInvoiceDetails($enroll_id);
$extINTL = extension_loaded('intl') && class_exists('NumberFormatter');
if ($extINTL == true) {
	$spellout = new NumberFormatter("en", NumberFormatter::SPELLOUT);
}
if (!empty($allocations) && is_array($allocations) && count($allocations)) {
	?>
	<section class="panel">
